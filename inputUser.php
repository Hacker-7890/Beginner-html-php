...
//echo "<br>";
 ...
 if (isset($_POST['cadastrar'])) {
   ...
   $person=base64_encode($userCript).".php";
   $d=base64_encode($userCript);
   $profile=$d."profile/";
   $msg=$d."msg/";
   setcookie("userLink",$d);
   if (!is_dir($db)) {
      mkdir($db,0755,true);
   }
   echo "<br>";
   echo "string";
   if (is_file($db.$confirm)) {
      echo "exist";
      echo "<script>alert(123456)</script>";
...
?> 
      ...
      $personCO=fopen($person, "x+");
      fwrite($personCO,$conteudos);
      fclose($personCO);
